  ...
  const signup = Joi.object({
    ...
    email: Joi.string()
      ...
      role: Joi.string().default("user").required().messages({
        'string.empty': "select role"
      }),
    phone: Joi.string().required().min(10).max(15).trim().pattern(/^\+?\d{10,15}$/).messages({
    ...
    .messages({
        ...
        'string.pattern.base': 'Password must be 5–15 characters long and include at least one letter and one number',
    })
  });
